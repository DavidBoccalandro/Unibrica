...
@Component({
  selector: 'app-line-chart-reversals-clients',
  templateUrl: './line-chart-reversals-clients.component.html',
  styleUrls: ['./line-chart-reversals-clients.component.scss']
})
export class LineChartReversalsClientsComponent {
  lineChartData: any[] = [];
  ...
  adaptStatisticsToChartData(response: StatisticsResponse[]): ChartData {
    ...
    response.forEach((stat) => {
        Object.keys(stat.statistics.totalDebitAmount).forEach((day) => {
          const value = Number(stat.statistics.totalReversalAmount[day]);
    ...
    const finalStatistics = [
      ...responseFiltered,
      { clientName: 'Total', statistics: { totalReversalAmount: totalStatistics } },
    ];
    ...
    return {
      ...
      datasets: finalStatistics.map((stat, index) => {
        return {
          ...
          data: labels.map((day: string) => Number(stat.statistics.totalReversalAmount[day])), // Asegurarse de mapear las fechas correctamente
          fill: false,
